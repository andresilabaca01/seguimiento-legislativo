...
# -*- coding: utf-8 -*-
"""
Scraper ABIF v6: agrega soporte robusto para Resultados de Cámara y sesiones realizadas.
...
6) Actualiza movimientos de proyectos ya seguidos si aparecen en citaciones/resultados, incluso si el boletín está duplicado en el JSON.
7) Conserva manualmente los resúmenes, relevancias y datos existentes del monitor.
8) Si Cámara devuelve 403 desde GitHub Actions, usa fallback opcional con curl_cffi y Jina Reader para páginas públicas.

Uso local/GitHub Actions:
  python scraper_abif_v6.py --input proyectos.json --output proyectos.json
"""
...
HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; ABIF-Monitor-Legislativo/6.0; +https://github.com/)",
...
def request_text(url: str, params: Optional[dict] = None, timeout: int = 25) -> str:
    """Lee una URL con varias estrategias.

    GitHub Actions a veces recibe 403 desde camara.cl aunque el sitio sea público.
    Por eso se intenta:
      1) requests normal con headers de navegador,
      2) curl_cffi con impersonación Chrome,
      3) Jina Reader como fallback para páginas públicas de Cámara.

    El fallback de Jina se puede desactivar con USAR_JINA_FALLBACK=0.
    """
    last = None
    ...
    # 2) curl_cffi impersonando Chrome. Ayuda cuando el bloqueo depende del fingerprint TLS/HTTP.
    try:
    ...
    # 3) Fallback Jina Reader para páginas públicas de Cámara.
    usar_jina = os.getenv("USAR_JINA_FALLBACK", "1").strip() != "0"
    es_camara_publica = "camara.cl/legislacion/comisiones/" in url or "www.camara.cl/legislacion/comisiones/" in url
    if usar_jina and es_camara_publica and params is None:
        try:
...
def proyectos_por_boletin_all(proyectos: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    """Mapea boletín normalizado a TODOS los proyectos coincidentes.

    Esto corrige un problema práctico del monitor: algunos boletines están duplicados
    en la base local/remota. En v5 se actualizaba solo el último duplicado, por lo
    que el usuario podía seguir viendo una tarjeta antigua.
    """
...
def recent_week_links(index_url: str, max_links: int = 8) -> List[str]:
    ...
    # Si el contenido llegó como Markdown desde Jina, BeautifulSoup puede no encontrar <a>.
    # Extraemos URLs absolutas/relativas directamente del texto.
    for m in re.finditer(r"https://www\.camara\.cl/legislacion/comisiones/(?:citaciones|resultados)_semana\.aspx\?prmSemana=20\d{2}-\d{1,2}", html):
        links.append(m.group(0))
    for m in re.finditer(r"(?:citaciones|resultados)_semana\.aspx\?prmSemana=20\d{2}-\d{1,2}", html):
        links.append(urljoin(index_url, m.group(0)))
...
def plain_text_from_response(raw: str) -> str:
    """Normaliza HTML o Markdown/texto a una sola cadena searchable."""
    if re.search(r"<html|<body|<table|<div|<span|<p|<br", raw or "", re.I):
        soup = BeautifulSoup(raw, "html.parser")
        return re.sub(r"\s+", " ", soup.get_text(" ")).strip()
    # Markdown/texto plano, típicamente fallback Jina.
...
def scan_html_camara(data: Dict[str, Any]) -> Tuple[int, List[str]]:
    proyectos = data.setdefault("proyectos", [])
    candidatos = data.setdefault("candidatos", [])
    pmap_all = proyectos_por_boletin_all(proyectos)
    existentes = set(pmap_all.keys()) | {norm_boletin(c.get("boletin", "")) for c in candidatos}
    logs = []
    duplicados = {b: ps for b, ps in pmap_all.items() if len(ps) > 1}
    if duplicados:
        logs.append("Boletines duplicados que se actualizarán en todas sus tarjetas: " + ", ".join(sorted(duplicados.keys())[:20]))
    cambios = 0
    ...
    for tipo, url in urls_tipo:
        try:
            ...
            text = plain_text_from_response(html)
            ...
            for m in boletines:
                ...
                if b in pmap_all:
                    for pp in pmap_all[b]:
                        if add_hist(pp, fecha, base):
                            cambios += 1
                else:
...
def main():
    ...
    data["version"] = dt.datetime.now(TZ).strftime("%Y-%m-%d-%H%M-abif-v6")
